...
def visualize_images(images, num_rows=None, path=None):
    '''
    ...
    scipy.misc.imsave(path, grid_visualizer)
    return grid_visualizer

def visualize_histogram(true_bpd, false_bpd, path=None):
    plt.figure()
    plt.hist(true_bpd, 100, alpha=0.5, label='correct samples', density=True)
    plt.hist(false_bpd, 100, alpha=0.5, label='wrong samples', density=True)

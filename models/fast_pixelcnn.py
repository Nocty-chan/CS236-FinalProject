...
def model_spec(row_input,
    ...
    with arg_scope(
        ...
        with arg_scope(
            ...
            xent = fast_nn.compute_xent(ref_img, x_out) / np.log(2.0)

            if pxl_fixed:
                adpt_eps = tf.cast(decode_eps * tf.to_float(xent > 3.0), dtype=tf.int32)[..., None]
            else:
                adpt_eps = decode_eps

            if not denoise:
                sample = nn.sample_from_softmax(ref_img, x_out)
            else:
                sample = nn.decode_from_softmax_with_box_constraint(ref_img, x_out, adpt_eps)

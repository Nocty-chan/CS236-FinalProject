...
class ACWGAN_GP(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir,
        ...
        if dataset_name == 'mnist':
            ...
            self.learning_rate = 0.0002 if learning_rate is None else learning_rate
            (train_x, train_y), _ = tf.keras.datasets.cifar100.load_data()
            self.data_X = train_x / 255.
            self.data_y = np.reshape(train_y, (-1))
    ...
    def generator(self, z, y, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            elif self.dataset_name == 'svhn' or self.dataset_name == 'cifar100':
    ...
    def train(self):
        ...
        # summary writer
        self.writer = tf.summary.FileWriter(self.log_dir + '/' + self.model_name + '_' + self.dataset_name, self.sess.graph)
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                decay = np.maximum(0.0, 1. - counter / (self.n_iters - 1))
                ...
                batch_y = self.data_y[idx * self.batch_size * self.disc_iters : (idx + 1) * self.batch_size * self.disc_iters]
                ...
                if np.mod(counter, 300) == 0:
                    ...
                    save_images(samples[:manifold_h * manifold_w, :, :, :], [manifold_h, manifold_w],
                                './' + check_folder(
                                    self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_' + self.dataset_name + '_train_{:02d}_{:04d}.png'.format(
                                    epoch, idx))
    ...
    def visualize_results(self, epoch):
        ...
        save_images(samples[:image_frame_dim*image_frame_dim,:,:,:], [image_frame_dim, image_frame_dim],
                    check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_' + self.dataset_name + '_epoch%03d' % epoch + '_test_all_classes.png')
        ...
        for l in range(self.len_discrete_code):
            ...
            samples = self.sess.run(self.fake_images, feed_dict={self.z: z_sample, self.y: y})
            save_images(samples[:image_frame_dim*image_frame_dim,:,:,:], [image_frame_dim, image_frame_dim],
                        check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_' + self.dataset_name + '_epoch%03d' % epoch + '_test_class_%d.png' % l)
        ...
        save_images(canvas, [n_styles, self.len_discrete_code],
                    check_folder(self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_' + self.dataset_name + '_epoch%03d' % epoch + '_test_all_classes_style_by_style.png')
    ...
    def save(self, checkpoint_dir, step):
        ...
        self.saver.save(self.sess, os.path.join(checkpoint_dir, self.model_name + '_' + self.dataset_name + '.model'), global_step=step)

    def load(self, checkpoint_dir):
        import re
        print(" [*] Reading checkpoints...")
        checkpoint_dir = os.path.join(checkpoint_dir, self.model_dir, self.model_name + '_' + self.dataset_name)
